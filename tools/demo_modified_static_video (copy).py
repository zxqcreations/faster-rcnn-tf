...
def vis_detections(im, im2, dets, thresh=0.5):
    ...
    if not tmp_frm == []:
        ...
        cv2.imwrite('/home/test/Assigned_video/obj/obj_{:s}.jpg'.format(str(cur_ind).zfill(4)), t_im)
        cv2.imwrite('/home/test/Assigned_video/obj_inf/obj_{:s}.jpg'
                    .format(str(cur_ind).zfill(4)), t_imf)
        cur_ind += 1
...
if __name__ == '__main__':
    ...
    for i in range(0, 7):
        cap = cv2.VideoCapture('/home/test/video_source/'+str(i)+'.avi')
        cap_inf = cv2.VideoCapture('/home/test/video_source/i_'+str(i)+'.avi')
        # if you cannot connect to your camera, try to change this value, e.g., change 0 to 1

        while(True):
            if not cap.isOpened():
                print('failed to open {:d} stream, reopen this script may correct it'.format(i))
                break
    ...
    l = list(map(int, np.linspace(0,cur_ind,num=cur_ind,dtype=np.int16)))
    print(l)
